...
// Helper to get issues from localStorage
...
export const addIssue = (
  issueData: Omit<CivicIssue, 'id' | 'createdAt' | 'status' | 'userId' | 'userEmail' | 'username' | 'acknowledgedAt' | 'resolvedAt' | 'rating'>,
  user: User
): CivicIssue => {
  const issues = getIssues();
...
export const updateIssueStatus = (id: string, newStatus: Status, adminUser: User): CivicIssue | null => {
  const issues = getIssues();
...
export const addFeedbackToIssue = (id: string, feedback: string): CivicIssue | null => {
  ...
  const admin = findAdminByDepartment(updatedIssue.department);
  if (admin) {
    addNotification(admin.id, `New feedback received for the resolved issue: "${updatedIssue.title}".`, NotificationType.FeedbackReceived);
  }
...
export const getIssuesByDepartment = (department: Department): CivicIssue[] => {
  const issues = getIssues();
  return issues.filter(issue => issue.department === department);
};

// Get issues by status
export const getIssuesByStatus = (status: Status): CivicIssue[] => {
  const issues = getIssues();
