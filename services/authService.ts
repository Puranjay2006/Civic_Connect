...
export const login = (identifier: string, password: string): User | null => {
  ...
  if (userIndex !== -1 && users[userIndex].passwordHash === pseudoHash(password)) {
    // Update login count and mark as returning user
    const updatedUser = {
      ...users[userIndex],
      isNewUser: false,
      loginCount: (users[userIndex].loginCount || 1) + 1,
    };
...
export const loginAsSuperAdmin = (passkey: string): User => {
    ...
    if (!adminUser) {
        // Super admin user not found, creating one
...
export const loginAsDepartmentAdmin = (department: Department, passkey: string): User => {
    ...
    if (!deptAdmin) {
        // Department admin not found, creating one
...
export const markNotificationsAsRead = (userId: string): User | null => {
    ...
    // Create new notification objects with read = true
    const updatedNotifications = users[userIndex].notifications.map(n => ({ ...n, read: true }));
    users[userIndex] = { ...users[userIndex], notifications: updatedNotifications };
    saveUsers(users);

    const currentUser = getCurrentUser();
    if (currentUser && currentUser.id === userId) {
        // Return a completely new user object to trigger React re-render
        const updatedUser = { 
            ...users[userIndex],
...
export const deleteNotification = (userId: string, notificationId: string): User | null => {
    const users = getUsers();
    const userIndex = users.findIndex(u => u.id === userId);
    if (userIndex === -1) return null;

    // Filter out the notification to delete
    const updatedNotifications = users[userIndex].notifications.filter(n => n.id !== notificationId);
    users[userIndex] = { ...users[userIndex], notifications: updatedNotifications };
    saveUsers(users);
...
export const deleteAllNotifications = (userId: string): User | null => {
    const users = getUsers();
    const userIndex = users.findIndex(u => u.id === userId);
    if (userIndex === -1) return null;

    // Clear all notifications
    users[userIndex] = { ...users[userIndex], notifications: [] };
    saveUsers(users);
